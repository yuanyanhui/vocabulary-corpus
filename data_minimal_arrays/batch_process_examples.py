...
SOURCE_DIR = "source"
UPDATED_DIR = "updated"
CHUNK_SIZE = 2  # Process files in pairs to optimize API calls while managing complexity
...
def main():
    ...
    # Iterate through the files in steps of 2
    for i in tqdm(range(0, len(files_to_process), CHUNK_SIZE), desc="Processing file pairs"):
        chunk = files_to_process[i:i + CHUNK_SIZE]
        
        try:
            combined_words = []
            
            # 1. Read and combine files in the current chunk
            for filename in chunk:
                source_path = os.path.join(SOURCE_DIR, filename)
                with open(source_path, "r", encoding='utf-8') as f:
                    data = json.load(f)
                    # Assuming the JSON contains a list of words. 
                    if isinstance(data, list):
                        combined_words.extend(data)
                    else:
                        # Fallback if the JSON structure is a dictionary/object
                        combined_words.append(data)
            
            # 2. Generate a combined output filename
            if len(chunk) == 2:
                name1 = chunk[0].replace('.json', '')
                name2 = chunk[1]
                output_filename = f"{name1}_and_{name2}"
            else:
                # Handles the edge case where there is an odd number of files remaining
                output_filename = chunk[0] 
                
            updated_path = os.path.join(UPDATED_DIR, output_filename)

            # 3. Process with LLM
            prompt = generate_batch_prompt(combined_words)
            ...
            # 5. Record ALL files in this chunk as successfully processed
            for filename in chunk:
                save_processed_file(filename)

        except json.JSONDecodeError:
            tqdm.write(f"\nWarning: Failed to decode JSON for chunk {chunk}. Response from API may be invalid.")
            break
        except Exception as e:
            tqdm.write(f"\nAn unexpected error occurred for chunk {chunk}: {e}")
            break
